...
matrix GS_bs_stan(
  ...
  // Build indicator matrix as dense
  matrix[N, Nintervals] M1 = rep_matrix(0, N, Nintervals);
  for (i in 1:N) {
    for (j in 1:Nintervals) {
      if (allknots[j] <= x[i] && x[i] < allknots[j + 1]) {
        M1[i, j] = 1.0;
      }
    }
  }
  // Handle last knot (x == bknots[2])
...
// Main B-spline function (dense, robust)
matrix GS_ns_stan(
  vector x, vector knots, vector bknots, 
  vector fullknots, vector allknots, 
  int N, int degree, int ord, 
...
matrix GS_nsp_call_stan(
  vector x, vector knotsx, vector bknotsx, 
