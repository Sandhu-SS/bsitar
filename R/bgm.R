...
#'  mi(sdy) ~ ..)} where \code{mi(sdy)} is passed to the [brms::brmsformula()]
...
#'  formulation as allowed by the [brms::lf()] and [brms::nlf()] syntax. Both
#'  [brms::lf()] and [brms::nlf()] offer flexibility in centering of predictors
...
#'  
#'@param autocor_formula Formula to set up the modelling of residuals
...
#'  of the response variable (See [brms::prior()]). The \code{bsitar} package
...
#'
#'@param get_priors_eval An optional logical (default \code{FALSE}) to get priors
...
bgm <- function(x,
                   ...
                   b_prior_beta = normal(0, 3, autoscale = FALSE),
