...
#'   (\emph{SITAR}). The \emph{SITAR} model is a nonlinear mixed effects model
#'   that has been used extensively to summarize growth processes (such as
...
#'   Besides Bayesian implementation of the \emph{SITAR} model, the \pkg{bsitar}
#'   package greatly enhances the modelling capabilities of the already existing
#'   \pkg{sitar} package. For example, in addition to the univariate analysis
#'   (i.e, modelling a single outcome), the \pkg{bsitar} allows
#'   univariate-by-subgroup and multivariate model fitting. In
#'   univariate-by-subgroup analysis, a single outcome is analysed
#'   simultaneously for subgroups defined by a factor variable such as gender.
#'   An advantage of such approach is that posterior draws for each sub group
#'   are part of a single model object that makes it possible to compare
#'   coefficients across groups and test hypotheses. The multivariate analysis 
#'   involves simultaneous joint modelling of two or more outcomes.
#'  
...
#'  underlying population average curve via a set of (typically) three random
#'  effects, the \code{size}, \code{timing} and \code{intensity}. Additionally,
#'  a slope parameter can be included as a random effect to model the
#'  variability in the adult slope (rate) of the growth curve (See
#'  [sitar::sitar()] for details). The concept of shape invariant model (SIM)
#'  was first described by \insertCite{Lindstrom1995;textual}{bsitar} and
#'  later used by \insertCite{Beath2007;textual}{bsitar} to model infant growth
#'  data (birth to 2 years). The current version of the \emph{SITAR} model is 
#'  developed by \insertCite{Cole2010;textual}{bsitar} and has been used 
#'  extensively for modelling growth data
#'  \insertCite{@see @nembidzaneUsingSITARMethod2020; 
#'  @mansukoskiLifeCourseAssociations2019; @coleFiftyYearsChild2018; 
#'  @riddellClassifyingGestationalWeight2017; @Sandhu2020}{bsitar}. 
#'  
#'  The frequentist version of the \emph{SITAR} model can be fit by an already
#'  available R package, the \pkg{sitar} \insertCite{R-sitar}{bsitar}. The
#'  Bayesian implementation of the \emph{SITAR} model in \pkg{bsitar} package is
#'  same as the \pkg{sitar} package with the exception that unlike the
#'  \pkg{sitar}, package which uses the B spline basis for the natural cubic
#'  spline design matrix by calling the [splines::ns()] 
#'  \insertCite{R-splines}{bsitar}, the \pkg{bsitar} package constructs spline 
#'  design matrix by using the truncated power basis approach as described by
#'  \insertCite{harrell2001regression;textual}{bsitar}, and implemented in 
#'  [Hmisc::rcspline.eval()] \insertCite{@see @R-Hmisc}{bsitar}.
...
#'  included in the \code{functions} block of the **Stan** program and hence 
#'  compiled (via the c++) during the model fit.
#'  
...
#'  \insertCite{Cole2010}{bsitar}, enforces inclusion of parameter
...
#'  \emph{SITAR} model. For the three parameter version of the \emph{SITAR}
#'  model (default), the fixed effects structure (i.e., population average
#'  trajectory) is specified as \code{fixed = 'a+b+c'} whereas the random
#'  effects structure that captures the deviation of individual specific
...
#'  
#'  The \pkg{sitar} package internally depends on another R package, the
...
#'  (such as the timing and intensity of the growth spurt). For prior
...
#'  for auto scaling of the scale parameter. While \pkg{rstanarm} earlier used
#'  to set \code{autosclae} as 2.5 (recently authors changed this behavior to
#'  \code{FALSE}), the \pkg{brms} package sets it as \code{1.0} or \code{2.5}
#'  depending on the standard deviation of the response variable (See
#'  [brms::prior()]). The \pkg{bsitar} package, on the other hand, offers full
#'  flexibility in choosing the scale factor as any real number (e.g., 
#'  \code{autosclae = 2.5}). When \code{autosclae = TRUE}, \code{2.5} is the
#'  default scaling factor. We strongly recommend to go through the well
#'  documented details on prior specifications used in \pkg{brms} and
#'  \pkg{rstanarm} packages.
#'  
#'  Furthermore, like \pkg{brms} package, the \pkg{bsitar} package offers a
#'  range of tools to evaluate the model fit that include posterior predictive
#'  check (see [brms::pp_check()]) and the leave one out (\code{loo}) cross
#'  validation (see [brms::loo()]) Also, the excellent post-processing support
#'  offered by the \pkg{brms} package is further augmented via many custom
#'  functions written exclusively for the \pkg{bsitar} package. This allows for
#'  estimation and visualization of population average and individual specific
#'  distance (increase in size) and velocity (change in rate of growth), and
#'  estimation of population average and individual specific growth parameters
...
#'  fittings: \code{univariate}, \code{univariate_by} and \code{multivariate}. A
#'  \code{univariate} involves a single model applied to an outcome whereas both
#'  \code{univariate_by} and \code{multivariate} comprise two or more sub
#'  models. The \code{univariate_by} fits two or more separate sub models for a
#'  single outcome variable defined by a factor variable (e.g, sex). The data
#'  are typically stacked and the factor variable is used to set-up the sub
#'  models via the \code{'subset'} option available in the [brms::brm()]. The
#'  \code{multivariate} model allows simultaneous modelling of two or more
#'  outcomes with joint a distribution of the random effects. For both
#'  \code{univariate_by} and \code{multivariate} models, the \pkg{bsitar}
#'  package allows full flexibility in specifying separate predictor variables
#'  (\code{x}), individual identifiers (\code{id}), degree of freedom
#'  (\code{df}) for design matrix as well as priors and the initial values.
#'  Furthermore, to enhance the ease of specifying different options and to make
#'  it user-friendly, there is no need to enclose the character strings
#'  option(s) in single or double quotes. For example to specify the
...
#'  The key points in prior specification that are applicable for all parameters
#'  are highlighted below. For full details on prior specification, please see
#'  See [brms::prior()].
#'  \itemize{
...
#'  \code{'ymedian'}, \code{'ysd'}, and \code{'ymad'} are not allowed. The
#'  Option \code{'lm'} for the location parameter sets covariate(s) coefficient
#'  obtained from the simple linear model fit to the data. Note that option
#'  \code{'lm'} is allowed only for the \code{a_cov_prior_beta} and not for the
#'  covariate(s) included in the other fixed or random effect parameters.
#'  Lastly, separate priors can be specified for sub models when fitting
#'  \code{univariate_by} and \code{a_prior_beta} models (see
#'  \code{a_prior_beta}).
#' 
...
#'  fixed effect parameter, \code{s}. However, as described earlier, (see
#'  \code{s_formual}), the \emph{SITAR} model does not allows for inclusion of
#'  covariate(s) in the spline design matrix. If and when covariate(s) are
#'  specified (see \code{s_formual}), the approach of setting priors for the
#'  covariate(s) included in the parameter, \code{s} via \code{s_cov_prior_beta}
#'  is same as described earlier for the fixed effect parameter \code{a} (see
...
#'  the variance) and not on the variance itself. The approach of setting the
#'  prior is same as described earlier for the fixed effect parameter, \code{a}
#'  (See \code{a_prior_beta}) with the exception that location parameter is
#'  always zero. The lower bound \code{0} is automatically set by the
...
#'  parameter \code{sigma}. Note that this argument is evaluated only when both
...
#'  and +1. For the unstructured residual correlation structure, a separate
#'  argument \code{autocor_prior_unstr_acor} is used to specify the priors (see
#'  below).
#'  
...
#'   only allowed distribution is the \code{lkj}. See \code{gr_prior_cor} below
#'   for details on setting up the \code{lkj} prior.
#'
#'@param gr_prior_cor Specify priors for the correlation parameter(s) of
#'  group-level random effects. The only allowed distribution is \code{lkj} that
#'  is specified via a single parameter \code{eta} (see [brms::prior()] for
#'  details).
#'  
...
#'  more levels of hierarchy. The approach is same as described above (See
#'  \code{gr_prior_cor}).
#'
#'@param sigma_prior_cor Specify priors for the correlation parameter(s) of
#'  distributional random effects \code{sigma}. The only allowed distribution is
#'  \code{lkj} (see \code{gr_prior_cor} for details). Note that currently
...
#'  when fitting a multivariate model. The only allowed distribution is
#'  \code{lkj} (see \code{gr_prior_cor} for details).
#'
...
#'  Options available are \code{'0'}, \code{'random'} and \code{'prior'}. In
#'  addition, user can specify \code{'ymean'} and \code{'ymedian'} to set
#'  initial as the mean or the median of the response variable. Also, option
#'  \code{'lm'} can be used to set coefficients obtained from the simple linear
#'  model fitted to the data as initial values for the fixed effect parameter,
#'  \code{a}. Note that this is similar to the location parameter for prior on
#'  the fixed effect parameter \code{a} (see \code{a_prior_beta} for details).
#'  These options (\code{'ymean'}, \code{'ymedian'}, and \code{'lm'}) are
...
#'  See \code{a_init_beta} for details.
#'
...
#'  (spline coefficients). Options available are \code{'0'}, \code{'random'},
...
#'  \code{'random'}, \code{'prior'} and \code{'lm'}. The option \code{'lm'} is
#'  available only for the \code{a_cov_init_beta} and not for the covariate(s)
#'  included in other fixed effect parameters \code{b}, \code{c}, or \code{d}.
#'
#'@param b_cov_init_beta Initial values for covariate(s) included in the fixed
#'  effect parameter, \code{b}. See \code{a_cov_init_beta} for details.
#'
#'@param c_cov_init_beta Initial values for covariate(s) included in the fixed
#'  effect parameter, \code{c}. See \code{a_cov_init_beta} for details.
#'
#'@param d_cov_init_beta Initial values for covariate(s) included in the fixed
#'  effect parameter, \code{d}. See \code{a_cov_init_beta} for details.
#'  
#'@param s_cov_init_beta Initial values for covariate(s) included in the fixed
#'  effect parameter, \code{s} (spline coefficients). See \code{a_cov_init_beta}
#'  for details. The option \code{'lm'} will set the spline coefficients
#'  obtained from the simple linear model fitted to the data. Note that
...
#'  random effect parameter, \code{a}. Options available are \code{'0'},
...
#'  random effect parameter, \code{b}. See \code{a_init_sd} for details.
#'
...
#'  random effect parameter, \code{d}. See \code{a_init_sd} for details.
#'  
...
#'  random effect parameter, \code{b}. See \code{a_cov_init_sd} for details.
#'
#'@param c_cov_init_sd Initial values for the covariate(s) included in the
#'  random effect parameter, \code{c}. See \code{a_cov_init_sd} for details.
#'
#'@param d_cov_init_sd Initial values for the covariate(s) included in the
#'  random effect parameter, \code{d}. See \code{a_cov_init_sd} for details.
#'
#'@param sigma_init_beta Initial values for the fixed effect distributional
#'  parameter, \code{sigma}. Options available are \code{'0'}, \code{'random'}
#'  and \code{'prior'}.
#'
...
#'  as described earlier for the group level random effect parameters such as
#'  \code{a} (See \code{a_init_sd} for details).
#'
#'@param sigma_cov_init_sd Initial values for the covariate(s) included in the
#'  distributional random effect parameter, \code{sigma} (See
...
#'  parameter, \code{sigma}. Options available are \code{'0'}, \code{'random'}
#'  and \code{'prior'}. In addition, options \code{'lme_rsd'} and
#'  \code{'lm_rsd'} can be used as follows. The \code{lme_rsd} sets initial
#'  value based on the standard deviation of residuals obtained from the linear
#'  mixed model (\code{nlme::lme()}) fitted to the data. The initial value set
#'  by the \code{'lm_rsd'} is the square root of the residual variance from the
#'  simple linear model applied to the data. Note that in case
#'  \code{nlme::lme()} fails to converge, then option \code{'lm_sd_a'} is set
#'  automatically. The argument \code{rsd_init_sigma} is evaluated when
...
#'  \code{dpar_formula} is not \code{NULL}.
#'
#'@param dpar_cov_init_sigma Initial values for the covariate(s) included in the
#'  distributional parameter, \code{sigma}. Allowed options are \code{'0'},
#'  \code{'random'}, and \code{'prior'}.
#'
...
#'  autocorrelation parameters. Allowed options are \code{'0'}, \code{'random'},
#'  and \code{'prior'}. Note that the approach to set initials for
#'  \code{autocor_init_unstr_acor} is identical to the \code{gr_init_cor}.
#'
#'@param mvr_init_rescor Initial values for the residual correlation parameter
#'  when fitting a \code{multivariate} model. Allowed options are \code{'0'},
#'  \code{'random'}, and \code{'prior'}.
#'
#'@param r_init_z Initial values for the standardized group level random effect
#'  parameters. These parameters are part of the Non-Centered Parameterization
