...
#'
#' @param point_color Color used for the posterior mean points. If \code{NULL},
#'   the color is set internally to \code{"#1f4e79"} when \code{groupby} is
#'   \code{NULL}; otherwise, distinct colors are used for each level of the
#'   \code{groupby} variable.
#'
#' @param interval_color Color used for the posterior interval lines. If
#'   \code{NULL}, the color is set internally to \code{"#1f4e79"} when
...
#'   \pkg{patchwork}.
#'
#' @param refline Numeric value for the vertical reference line. Default is
#'   \code{0}.
#'
...
plot_caterpillar.bgmfit <- function(
  ...
  # multivariate ranef()
  summarize_randomeffects_mv <- function(
    re_list,
    # idvar,
    transform_parameter,
    transform_fun,
    ...
    lapply(re_list, summarize_randomeffects,
           probs = probs,
           robust = robust,
  ...
  # If your multivariate ranef() output is nested like:
  #   
  #   response
  # 
  # grouping factor
  ...
  # 
  # then use two loops:
  summarize_randomeffects_mv <- function(re_list, 
                                 # idvar,
                                 transform_parameter,
                                 transform_fun,
                                 probs = c(0.025, 0.975),
    ...
    lapply(re_list, function(resp_obj) {
      lapply(resp_obj, summarize_randomeffects,
             probs = probs,
             robust = robust,
             transform_parameter = transform_parameter,
             transform_fun = transform_fun)
  ...
  summarize_randomeffects_uv <- function(arr, 
                                      idvar, 
                                      transform_parameter, 
                                      transform_fun,
                                      probs = c(0.025, 0.975), 
  ...
  make_transform_map <- function(transform_parameter, transform_fun) {
    ...
    if (is.null(transform_fun)) {
      ...
      stop("transform_fun must be NULL, a function, or a named list of functions.")
    }
  ...
  summarize_randomeffects <- summarize_randomeffects_uv
  ...
  if(is.null(transform_parameter)) {
    re_list <- brms::ranef(model, summary = TRUE, probs = probs, robust = robust)
  } else {
    re_list <- brms::ranef(model, summary = FALSE)
    re_list <- summarize_randomeffects(arr = re_list, idvar = idvar_group,
  ...
  if (!idvar_group %in% names(re_list)) {
    stop("`Individual identifier` not found. Available groups: ",
         paste(names(re_list), collapse = ", "))
  }
  
  re_arr <- re_list[[idvar_group]]
  
  available_effects <- dimnames(re_arr)[[3]]
  
  if (is.null(effect)) {
    if ("a_Intercept" %in% available_effects) effect_loop <- "a_Intercept"
  } else if(effect == 'all') {
  ...
  for (effect in effect_loop) {
    if (!effect %in% available_effects) {
      stop("`effect` not found. Available effects: ", paste(available_effects, 
                                                            collapse = ", "))
    }
    ...
    if(is.null(gr_by)) {
      if (sort) {
        dframe <- dframe %>% 
          dplyr::mutate(idrow = forcats::fct_reorder(.f = .data[['idrow']], 
    ...
    if (is.null(label.x)) {
      # label.x     <- paste0("Random effect: ", effect)
      label.x     <- paste0("Estimate", "") # paste0("", effect)
    }
    if (is.null(label.y)) {
    ...
    #   label.title     <- effect # paste("Caterpillar plot")
    # }
    # if (is.null(label.subtitle)) {
    #   label.subtitle  <- NULL # effect # paste(effect, "by", idvar_group)
    # }
    
    # if (is.null(label.title) & is.null(label.subtitle)) {
    #   label.title     <- paste("Caterpillar plot")
    #   label.subtitle  <- effect # paste(effect, "by", idvar_group)
    # } else if (is.null(label.title) & !is.null(label.subtitle)) {
    #   label.title     <- effect # paste("Caterpillar plot")
    #   if(is.logical(label.subtitle)) {
    #     if(!label.subtitle) label.subtitle <- NULL 
    #     else label.subtitle  <- label.subtitle
    #   }
    # } else if (!is.null(label.title) & is.null(label.subtitle)) {
    #   if(is.logical(label.title)) {
    #     if(!label.title) label.title <- NULL 
    #     else label.title  <- label.title
    #   }
    ...
    if(layout == 'single') {
      if(!is.null(gr_by)) {
        dframe <- dframe %>%
          dplyr::mutate(!!as.name(gr_by) := forcats::fct_relevel(.data[[gr_by]])) %>%
    ...
    plot.o.d <- dframe %>%
      ggplot2::ggplot(., ggplot2::aes(x = .data[[Estimate.effect]], 
                                      y = .data[['idrow']])) +
      ggplot2::geom_vline(xintercept = refline_mark, linetype = 2, color = "grey50") +
...
# plot_caterpillar(model_selected, group = "id", effect = "a_Intercept")
