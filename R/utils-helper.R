...
insert_new_priors <- function(setdf_1, setdf_2) {
  ...
  setdf_1 <-
    setdf_1 %>%
    dplyr::mutate(index__x = interaction(class, coef, group, nlpar)) %>%
  ...
  vi_1 <- setdf_1 %>%
    dplyr::mutate(valid__x = ifelse(!(class == 'sd' &
                                        coef == ""), 1, 0)) %>%
    data.frame() %>%
    dplyr::filter(valid__x == 1) %>% dplyr::select(index__x) %>% unlist() %>%
    droplevels()
  
  setdf_1 <-
    setdf_1 %>% dplyr::filter(!(class == 'sd' & coef == ""))
  setdf_2 <-
    setdf_2 %>% dplyr::filter(!(class == 'sd' & coef == ""))
