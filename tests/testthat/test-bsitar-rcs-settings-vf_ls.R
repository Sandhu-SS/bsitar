...
test_that("bsitar works with rcs settings and sigma_formula_manual ls", {
  skip_on_cran()
  ...
  test_scode <- bsitar(x = age, y = height, id = id, 
                       ...
                       sigma_formula_manual =  
                         nlf(sigma ~ ls(x, sigmaa, sigmab, sigmac, 
  ...
  true_sbetas <- c(128.05,0.00, 0.01, 5.02, 4.28, -12.27, -13.25, -1.50, 0.51,
                   -1.51, 0.82, 0.34, 1.89, 0.28)
  ...
  expect_equal(round(test_gparms$Estimate[2], 2), 6.52,  tolerance = 0.01)
