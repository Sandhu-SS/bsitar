...
# 
# univariate_fit_cov <- readRDS("univariate_fit_cov.rds")
# multivariate_fit_cov <- readRDS("multivariate_fit_cov.rds")
# 
# 
# oopts <- options(future.globals.maxSize = 1.5 * 1e9)  ## 1.0 GB
...
# #
# # marginal_out_custom_mdT %>% dplyr::pull(estimate) %>% mean()
# #
# # marginal_out %>% dplyr::filter(term=='age') %>% dplyr::pull(estimate) %>% mean()
# #
# # head(marginal_out)
# 
...
# marginal_args[['future_splits']]  <- c(2,2)
# 
# 
# # devtools::load_all()
# # marginal_funcall <- marginal_comparisons
