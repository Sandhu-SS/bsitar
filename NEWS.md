...
- Changed the default value of init from NULL to 'random', so initial values are now randomly generated by Stan within a range controlled by init_r. When init_r = NULL, Stan draws initial values uniformly from −2,2 on the unconstrained scale. When init_r is specified (e.g., init_r = 0.5, the default), initials are drawn uniformly between −0.5,0.5 on the unconstrained scale. Users should be aware that this change in initialization may affect convergence, model fit, and posterior results (especially for sensitive/non-identified models or when adaptation is challenging). To revert to the earlier behavior, set init = NULL.
...
The default ``stype`` is now restored to ``rcs``
