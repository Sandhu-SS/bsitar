...
[@Cole2010]. The 'bsitar' package [@bsitar-package] can fit univariate model
(analysis of a single outcome), univariate_by model (analysis of a single
outcome separately for sub groups defined by a factor variable such as gender),
and multivariate model (joint modelling of two or more outcomes). A detailed
introduction to the *SITAR* model including biological and mathematical
description of the model, Bayesian estimation of the *SITAR* model, and the
default priors used in the 'bsitar' package are provided in the the [Bayesian
SITAR - An Introduction](Bayesian_SITAR_An_Introduction.html) vignette.

In this vignette, we fit univariate Bayesian *SITAR* model using 'bsitar'
package [@bsitar-package] and compare results with the non Bayesian *SITAR*
model fit via the 'sitar' package [@R-sitar]. Hereafter, the Bayesian and non
Bayesian model estimated by the 'sitar' and 'bsitar' package are referred to as
**sitar** and **bsitar** models, respectively. The purpose of this vignette is
to demonstrate that both 'sitar' and 'bsitar' packages perform similarly in
capturing population average as well as individual specific trajectories thereby
provide similar estimates of the timing and the intensity of the adolescent
growth spurt parameters. This is important to ensure that new 'bsitar' package
performs as expected (as compared to the *sitar* model) before we fit more
complex model (such as multivariate model) for which no direct comparison is
possible between *bsitar* and *sitar* models because 'sitar' package is
restricted to univariate model fitting.
...
*sitar* model results shown in the vignette included in the 'sitar' package 
[Fitting models with SITAR](https://CRAN.R-project.org/package=sitar), we 
analyze the exact same data. The data is a subset of the Berkeley data that 
include height measurements for individuals (between 8 and 18 years of age) 
