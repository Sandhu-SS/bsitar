...
modelling of two or more outcomes).

In this vignette, we focus on fitting a univariate SITAR model by using the
*bsitar* package. The mathematical description of the model along with prior
specifications are provided in the the [Bayesian SITAR - An
Introduction](Bayesian_SITAR_An_Introduction.html) vignette. We analyse Berkeley
dataset that comprise of repeated growth measurements made on males and
females from birth to adulthood. 

been analysed in the vignette [Fitting models
with SITAR](https://CRAN.R-project.org/package=sitar) as part of the *sitar*
package [@R-sitar]. The *sitar* package implements the non-Bayesian
(frequentist) version of the SITAR model. 

The Berkeley dataset comprise of repeated growth measurements made on males and
females from birth to adulthood. The 
height measurement during 

to explore the growth of height
in girls during puberty. The same data has been analysed in the vignette
[Fitting models with SITAR](https://CRAN.R-project.org/package=sitar) included
in the *sitar* package [@R-sitar]. The *sitar* package implements the
non-Bayesian (frequentist) version of the SITAR model. The Berkeley dataset is 
included in both *sitar* and *bsitar* packages.


If necessary, first install the *bsitar* package.
```{r lib, eval = FALSE, echo=TRUE,include=TRUE}
if (!require(bsitar)) install.packages('bsitar')
```

Load *bsitar* library and fit Bayesian SITAR model to the heights of girls from 8 to 18 years. The girls were measured every six months.
```{r, results='hide'}
library(bsitar)
...
```
 ...
 and their heights are shown (left) plotted as growth curves using different color to separate them. The curves after SITAR adjustment are also shown (right, as discussed below).
