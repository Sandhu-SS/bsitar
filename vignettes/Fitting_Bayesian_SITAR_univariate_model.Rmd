...
If required, install *bsitar* and *sitar* packages.
...
library(bsitar)
ff <- na.omit(berkeley[berkeley$sex == "Female" & 
...
# Fit Bayesian SITAR model (via 'bsitar' package) - note prefix 'b' to 'bfh1'.
...
Plot data, before and after SITAR adjustments
```{r plot_1, fig.cap= "bb" fig.show='hold',echo=TRUE,include=TRUE}
par(mar = c(4,4,1,1) + 0.1, cex = 0.8)
# Plot observed growth trajectories
sitar::mplot(x = age, y = height, id = id, data = ff, col = id, las = 1)
# Plot growth trajectories adjusted by the SITAR model
plot(fh1, opt = 'a', col = id, las = 1, xlim = sitar::xaxsd(), ylim = sitar::yaxsd())
# Plot growth trajectories adjusted by the Bayesian SITAR model
plot_curves(bfh1, 'a')
```
...
knitr::kable(nlme::ranef(fh1)[1:10, ], digits = 2, 
             caption = "Random effects for the first ten girls
             (SITAR model fit via 'sitar' package)", 
             label = NA,
             format = "html", align = 'c') %>% 
