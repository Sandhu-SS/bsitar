...
A detailed introduction to the *SITAR* model including biological and
mathematical description of the model, as well as its Bayesian estimation using
'bsitar' package has been provided in the [Bayesian SITAR - An
Introduction](Bayesian_SITAR_An_Introduction.html). The 'bsitar' package
[@bsitar-package] can fit univariate model (analysis of a single outcome),
univariate_by model (analysis of a single outcome separately for sub groups
defined by a factor variable such as gender), and multivariate model (joint
modelling of two or more outcomes). The mathematical description of the model
along with prior specifications are provided in the the [Bayesian SITAR - An
Introduction](Bayesian_SITAR_An_Introduction.html) vignette.

In this vignette, we fit a univariate Bayesian *SITAR* model using the 'bsitar'
package [@bsitar-package] and compare results with the non Bayesian
*SITAR* model fit via the 'sitar' package [@R-sitar]. Hereafter, the Bayesian and non
Bayesian model fits are referred to as **sitar** and **bsitar** models,
respectively. The purpose of this vignette is to demonstrate that both 'sitar'
and 'bsitar' packages perform almost identically in capturing population average
as well as individual specific trajectories thereby provide similar estimates of
the timing and the intensity of the adolescent growth spurt parameters. This is
important to ensure that new 'bsitar' package performs as expected before we fit
more complex model (such as multivariate model) for which no direct comparison
is possible with the 'sitar' package because it is restricted to univariate
model fitting.
...
'sitar' and 'bsitar' packages. To compare our findings directly with the
non-Bayesian (frequentist) model fit results shown in the vignette [Fitting
models with SITAR](https://CRAN.R-project.org/package=sitar) included in the
'sitar' package, we analyze the exact same data which is used in the [Fitting
models with SITAR](https://CRAN.R-project.org/package=sitar). The data is a
subset of the Berkeley data that include height measurements for individuals (between
8 and 18 years of age) collected every six months.
...
\newline
## Model fit
Fit *SITAR* (**sitar**) and *Bayesian SITAR* (**bsitar**) models to the height data for individuals  
...
\newline
## Model summary
Summary of *SITAR* model fit (**sitar**)
```{r summary_sitar, results='show', echo=FALSE,include=TRUE}
print(model_frequ)
```

Summary of Bayesian SITAR model fit ((**bsitar**))
...
\newline
Figure \@ref(fig:plota) (see below) shows a comparison of observed
individual-specific growth trajectories with those adjusted via the random
effects. As described earlier in the [Introduction], the random effects (size,
timing and intensity) align individual-specific growth trajectories towards a
common (population average) trajectory. The three growth patterns that *SITAR*
...
The *SITAR* model estimates size, timing and intensity for each individual as
random effects, and then uses them to adjust individual curves to the average.
Figure \@ref(fig:plota) shows adjusted curves are now superimposed and hence
most of the inter-individual variability has been captured. This is achieved
this by shifting the high curves down and the low curves up (i.e. adjusting for
size), and shifting the early curves later and the late curves earlier (timing).
For intensity, the steep curves are made shallower by stretching them along the
age scale, and the shallow curves are made steeper by shrinking them along the
age scale. This explains the name SITAR - Super Imposition by Translation And
Rotation. The curve adjustment involves translation (shifting the curves up/down
and left/right) and rotation (making them steeper or shallower), and the effect
is to superimpose the curves. The complexity of the spline curve's shape is
determined by the degrees of freedom, as set with the degree of freedom argument
in the **sitar** and **bsitar** call. In our example,  there are five degrees of
freedom.
...
To see the effect of SITAR adjustment in individual curves via random effects,
the Figure \@ref(fig:plotuadids) (below) shows observed trajectories, mean curve
(population average) as well as the tallest and shortest individuals before and
after adjustment for the **sitar** and **bsitar** models. The random effect
estimates for **sitar** and **bsitar** models shown in Table
\@ref(tab:tsidssitar) and Table \@ref(tab:tsidsbsitar) capture deviation in the
(size, timing and intensity for shortest and tallest from the population average
(mean) curve. Note that intensity c, multiplied by 100, is a percentage.
Results for both the models are very similar to each other. Results for the
**sitar** model are presented below.

For the shortest individual,  size is `r round(sitar_ranef_id_min$a, 2)` cm
smaller than mean and her age at peak height velocity is `r round(sitar_ranef_id_min$b, 2)` year earlier than average (timing) whereas her
growth rate is `r round(sitar_ranef_id_min$c*100, 2)` % faster than average
(intensity). In contrast, the size for tallest individual is `r round(sitar_ranef_id_max$a, 2)` cm larger than mean with age at peak height
velocity occurring `r round(sitar_ranef_id_max$b, 2)` year later than average
(timing) with growth rate `r round(sitar_ranef_id_max$c*100, 2)` % slower than
average (intensity).

The associations (correlations) between the random effects for the **sitar** and
**bsitar** models are shown in Figure \@ref(fig:sitarranefcorrplot) and Figure
\@ref(fig:bsitarranefcorrplot) whereas the corresponding correlation estimates
are shown in Table \@ref(tab:sitarranefcorr) and Table
\@ref(tab:bsitarranefcorr). Results show that timing is negatively correlated
with intensity but size is uncorrelated with timing and intensity indicating the
early puberty is associated with faster growth - and hence a shorter growth
spurt.
...
\@ref(tab:sitarraneffecid)) and (Table \@ref(tab:bsitarraneffecid)).


```{r plotuadids, echo=TRUE,include=TRUE, fig.width = 6, fig.height=4, fig.asp = 1.0, fig.cap="\\label{fig:plotuadids}Observed and adjusted curve (via random effects) curves along with the population averag curves for the **sitar** and **bsitar** models. The observed curves for all individuals are shown as solid light grey lines whereas the observed curves for the tallest and shortest individuals are displayed in solid black lines. The adjusted curves for the tallest and shortest individuals are shown as dotted black lines. The population average curve is shown as solid black line"}
...
\newline
```{r plotv, echo=TRUE,include=TRUE, fig.cap="\\label{fig:plotv}Population average velocity curves (change in growth rate with age) for the **sitar** and **bsitar** models"}
data_all_dv %>% 
...
\newline
```{r plotV2, echo=TRUE,include=TRUE, fig.cap="\\label{fig:plotV2}Individual specific velocity curves (change in growth rate with age) for the **sitar** and **bsitar** models"}
data_all_DV %>% 
  dplyr::mutate(groupby = interaction(id, Model) ) %>% 
  ggplot(., aes(x = age)) +
  geom_line(aes(x = age, y = velocity, group = groupby, 
...
Individual curves (Figure \@ref(fig:plotD2) and \@ref(fig:plotV2)) show that all
individuals experience a pubertal growth spurt but the timing of the spurt
varies between 10 and 14 years with varying growth pattern among individuals.
Some individuals are consistently taller than average and others consistently
shorter. While some start relatively short and become taller, others have
opposite growth pattern i.e., they are taller at an early age and achieve less
height in later years as compared to those who were short at an early age.
...
velocity curves for the **sitar** and **bsitar** models are very similar to each other
indicating that they both models provide similar estimates for the increase in
height with age and the change in growth velocity as a function of age. Almost
identical velocity curves for the **sitar** and **bsitar** models suggest that they
capture similar timing (i.e., age at peak growth velocity, APGV) and the
intensity (i.e.,peak growth velocity, PGV) growth parameters. This is confirmed
by the results shown below (Table \@ref(tab:tabapgv))
...
\newline
### Individual specific (mean) estimates
Like population average curves (Figure \@ref(fig:plotd) and Figure
\@ref(fig:plotv)), individual specific distance and velocity curves displayed in
Figure \@ref(fig:plotD2) and Figure \@ref(fig:plotV2) show that both **sitar** and
**bsitar** models estimate similar trajectories for Individuals and hence provide
almost identical estimates of timing (APGV) and the intensity (PGV). The APGV and PGV estimates for the first ten individuals estimated by the **sitar** and **bsitar**
models are shown below in (Table \@ref(tab:sitarraneffecidapv)) and (Table
\@ref(tab:bsitarraneffecidapv)).
