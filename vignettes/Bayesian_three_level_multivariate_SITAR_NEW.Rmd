---
title: "Bayesian SITAR model - Three level multivariate model new"
...
citecolor: magenta
vignette: >
  %\VignetteIndexEntry{Bayesian SITAR model - Three level multivariate model new}
...
[rstanrm](https://cran.r-project.org/web/packages/rstanarm/vignettes/priors.html)
...
#### Three level multivariate model
...
data <- data %>% 
  ...
  group_by(sex, study, class, id, age) %>% 
  filter(row_number(age) == 1) %>% # remove duplicate age
  group_by(sex, study, class, id) %>% 
...
data <- data %>% filter(age >= 6 & age <= 20) %>% tidyr::drop_na()

data <- data %>% filter(class == "Class I") %>% droplevels() %>% tidyr::drop_na()

data_male <- data %>% filter(sex == 'Male') %>% droplevels() %>% tidyr::drop_na()

data_female <- data %>% filter(sex == 'Female') %>% droplevels() %>% tidyr::drop_na()

# remove Fels with one male
data_male <- data_male %>% filter(study != 'Fels') %>% droplevels() %>% tidyr::drop_na()

data_male <- dataset_in %>% mutate(id2 = id, study2 = study) %>% ungroup()
...
sdata <- bsitar(x = age, y = list(copad, copod), id = id, 
                ...
                sigma_formula_gr_str = ~ (1 + age | z | id2:study2) + (1 + age | z2 | study2),
                sigma_prior_beta = normal(0, 1, autoscale = FALSE),
                sigma_init_beta = 0,
                
                
                threads = threading(10, grainsize = 1),
                ...
                get_stancode = F,
                init = 0,
                # set_replace_priors = setup_new_priors,
                # init_custom = custom_initials,
...
sd_2_initials <- rep(0.1, sdata$M_2)
Cor_2_initials <- matrix(0, sdata$NC_2, sdata$NC_2)
diag(Cor_2_initials) <- 1.0
z_2_initials <- matrix(0, sdata$M_2, sdata$N_2)

sd_3_initials <- sd_2_initials
Cor_3_initials <- Cor_2_initials
z_3_initials <- z_2_initials

custom_initials <- list(sd_2 = sd_2_initials,
                        L_2 = Cor_2_initials,
                        z_2 = z_2_initials,
                        sd_3 = sd_3_initials,
                        L_3 = Cor_3_initials,
...
# initials should be a single list of list or equal to the number of chains
# i.e., the above custom_initials can be passed or below as list 

init_fun <- function(chain_id = 1) custom_initials
n_chains <- 1
custom_initials <- lapply(1:n_chains, function(id) init_fun(chain_id = id))


############################################ 
# set priors for level 3 random effects
############################################ 


# create list of new priors

# For lkj prior resp  should be empty

############################################
# sd prior arguments for first outcome
############################################
...
setnlpar <- 'a'
setcoef <- c('Intercept' )
setprior <- c('normal(0, 10)' )

x1 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

setnlpar <- 'b'
setcoef <- c('Intercept' )
setprior <- c('normal(0, 1)' )

x2 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

setnlpar <- 'c'
setcoef <- c('Intercept' )
setprior <- c('normal(0, 0.25)' )

x3 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)
...
x4 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

xp_copad <- list(x1, x2, x3, x4)



############################################
# sd prior arguments for second outcome
############################################
...
# Since brms does not allow resp specific LKJ prior, should assign it to only 
# one outcome

xp_copod <- list(x1, x2, x3)




############################################
# sigma prior arguments for first outcome
############################################
...
setnlpar <- ''
setcoef <- c('Intercept', 'age')
setprior <- c('normal(0, 1)' , 'normal(0, 0.25)' )

x1 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

xp_copad_sigma <- list(x1)
...
setnlpar <- ''
setcoef <- c('Intercept', 'age')
setprior <- c('normal(0, 2)' , 'normal(0, 0.5)' )

x1 = list(nlpar = setnlpar, prior = setprior ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

xp_copod_sigma <- list(x1)




xlist <- c(xp_copad, xp_copod, xp_copad_sigma, xp_copod_sigma)

setup_higher_priors <- utils::getFromNamespace("setup_higher_priors", "bsitar")

setup_new_priors <- setup_higher_priors(xlist)




####################################################################### 
# Fit 3 level model with above initials and priors for random effects
####################################################################### 


fit1 <- bsitar(x = age, y = list(copad, copod), id = id, 
               data = data_male,
               df = list(4, 4), 
               multivariate = T,
               # xoffset = list(12, 12),
