...
\\
\alpha_0   & \sim \operatorname{normal}(\ y_{mean},\  {y_{sd}},\ autoscale= TRUE) \\
\zeta_0   & \sim \operatorname{normal}(\ 0,\ 2.0, \ autoscale= FALSE) \\
\gamma_0  & \sim \operatorname{normal}(\ 0,\ 1.0, \ autoscale= FALSE) \\
\beta_1 \text{,..,} \beta_r & \sim \operatorname{normal}(\ 0, \ \mathbf {X_{scaled}},\ autoscale= TRUE) \\
\alpha_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {y_{sd}}, \ autoscale= TRUE) \\
\zeta_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {2}, \ autoscale= FALSE) \\
\gamma_j   & \sim \operatorname{normal_{Half}}(\ {0},\  {0.5}, \ autoscale= FALSE) \\
\sigma_\epsilon & \sim \operatorname{normal_{Half}}(\ {0},\  {y_{sd}}, \ autoscale= TRUE) \\
...
the timing parameter will cover range between 10 and 18 years when the predictor `age` is centered at 14 years. Depending on the mean age and whether the data correspond to males or females, the scale factor can be adjusted accordingly. The prior for the standard deviation of `b` parameter is also `normal`, with a mean of 0 and a standard deviation of $1.0$ (`normal(0, 2.0, autoscale = FALSE)`), implying that 95% of the distribution's mass for the individual variability in the timing 
parameter will cover $\pm 4.0$ years from the population average parameter `b`.

* The default prior for the population average intensity regression parameter `c` is 'normal' with a mean of `0` and a scale of `1.0`, i.e., `normal(0, 1.0, autoscale = FALSE)`. Since the intensity parameter is estimated on the `exp` scale, this prior implies that 95% of the distribution's mass will cover intensities between `exp(-2)` and `exp(2)`, i.e., between `0.1` and `7.4` unit/year. The prior for the standard deviation parameter (i.e., the random effect parameter `c`) is also 'normal', with a mean of `0` and a standard deviation of `0.5`, i.e., `normal(0, 0.5, autoscale = FALSE)` implying that 95% of the distribution for individual variation in intensity will cover `± exp(0.5)`, i.e., between `0.6` and `1.6` unit/year.
...
\end{bmatrix} \\
\\
\alpha_0^\text{Y1}   & \sim \operatorname{normal}(\ y^\text{Y1}_{mean},\  {y^\text{Y1}_{sd}},\ autoscale= TRUE) \\
\alpha_0^\text{Y2}   & \sim \operatorname{normal}(\ y^\text{Y2}_{mean},\  {y^\text{Y2}_{sd}},\ autoscale= TRUE) \\
\zeta_0^\text{Y1} & \sim \operatorname{normal}(\ 0, 2.0,\ autoscale= FALSE) \\
\zeta_0^\text{Y2} & \sim \operatorname{normal}(\ 0, 2.0,\ autoscale= FALSE) \\
\gamma_0^\text{Y1} & \sim \operatorname{normal}(\ 0, 1.0,\ autoscale= FALSE) \\
\gamma_0^\text{Y2} & \sim \operatorname{normal}(\ 0, 1.0,\ autoscale= FALSE) \\
\beta_1^\text{Y1} \text{,..,} \beta_r^\text{Y1} & \sim \operatorname{normal}(\ 0, \ \mathbf {X^\text{Y1}_{scaled}},\ autoscale= TRUE) \\
\beta_1^\text{Y2} \text{,..,} \beta_r^\text{Y2} & \sim \operatorname{normal}(\ 0, \ \mathbf {X^\text{Y2}_{scaled}},\ autoscale= TRUE) \\
\alpha_j^\text{Y1}   & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y1}_{sd}},\ autoscale= TRUE) \\
\alpha_j^\text{Y2}   & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y2}_{sd}},\ autoscale= TRUE) \\
\zeta_j^\text{Y1}  & \sim \operatorname{normal_{Half}}(\ {0},\ {2},\ autoscale= FALSE) \\
\zeta_j^\text{Y2} & \sim \operatorname{normal_{Half}}(\ {0},\ {2},\ autoscale= FALSE) \\
\gamma_j^\text{Y1} & \sim \operatorname{normal_{Half}}(\ {0},\ {0.5},\ autoscale= FALSE) \\
\gamma_j^\text{Y2} & \sim \operatorname{normal_{Half}}(\ {0},\ {0.5},\ autoscale= FALSE) \\
\sigma_{ij}^\text{Y1} & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y1}_{sd}},\ autoscale= TRUE) \\
\sigma_{ij}^\text{Y2} & \sim \operatorname{normal_{Half}}(\ {0},\  {y^\text{Y2}_{sd}},\ autoscale= TRUE) \\
