...
The default initial values follow Stan’s randomly generated initial values drawn
uniformly from −0.5,0.5 on the unconstrained scale. Users should note that this
behavior is different from Stan in terms of the range, which is -2.0 to 2.0 in
Stan but −0.5,0.5 in bsitar. Furthermore, the bsitar package allows full control
in specifying custom initial values for each of the fixed effects as well as the
random effects (variance covariance parameters as well as the residual variance)
via dedicated argument.
