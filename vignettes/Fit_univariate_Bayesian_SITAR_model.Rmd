...
females (between 8 and 18 years of age, every six months). However, to reduce 
the file size of the model object, we select a random sample of 20 individuals.
...
data <- berkeley_mdata
if(existsdata) {
  if(exists('berkeley_fdata')) data <- berkeley_fdata else stop("data does not exist")
} else {
...
if(existsfit) {
  if(exists('berkeley_ffit')) model_bayes <- berkeley_ffit else stop("model does not exist")
} else {
  model_bayes <-  bsitar(x = age, y = height, id = id, data = data, df = 5,
