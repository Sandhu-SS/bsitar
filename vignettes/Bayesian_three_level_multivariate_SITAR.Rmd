...
x1 = list(nlpar = 'a', prior = 'normal(0, 10)' ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

x2 = list(nlpar = 'b', prior = 'normal(0, 1)' ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

x3 = list(nlpar = 'c', prior = 'normal(0, 0.25)' ,
          class = setclass, coef  = setcoef, group = setgroup, 
          resp = setresp, lb = setlb,  ub = setub, dpar  = setdpar)

x4 = list(nlpar = '', prior = 'lkj(1)' ,
          class = 'cor', coef  = '', group = setgroup, 
          resp = '', lb = '',  ub = '', dpar  = setdpar)
...
x4 = list(nlpar = '', prior = 'lkj(1)' ,
          class = 'cor', coef  = '', group = setgroup, 
          resp = setresp, lb = '',  ub = '', dpar  = setdpar)
...
# model sigma fixed effects

fit2 <- bsitar(x = age, y = list(copad, copod), id = id, 
               data = data_male,
               df = list(4, 4), iter = 1000,
               multivariate = T,
               # xoffset = list(12, 12),
...
# model sigma fixed and random
# Here setting initials becomes challenge, therefore init = 0 or init = random

fit3 <- bsitar(x = age, y = list(copad, copod), id = id, 
               data = data_male,
               df = list(4, 4), iter = 1000,
               multivariate = T,
               # xoffset = list(12, 12),
               ...
               sigma_formula = ~ age + study,
               sigma_formula_gr = ~ age | ss | id,
               sigma_prior_beta = normal(0, 2, autoscale = FALSE),
               sigma_init_beta = 0,
