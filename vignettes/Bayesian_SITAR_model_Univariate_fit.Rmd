...
# For Distance and Velocity (DV) plots
data_sitar_D  <- plot(model_frequ, opt = 'D', xlim = sitar::xaxsd(), ylim = sitar::yaxsd(), returndata = TRUE)
# if(packageVersion('sitar') >= '1.4.0') this V no longer works
# Instead, a way out is to use predict
# But again there is a dim mismatch - so working hard to get data_sitar_V
# data_sitar_V  <- plot(model_frequ, opt = 'V', xlim = sitar::xaxsd(), ylim = sitar::yaxsd(), returndata = TRUE) 
if(packageVersion('sitar') >= '1.4.0') {
  data_sitar_V  <- predict(model_frequ, deriv = 1, level = 1, newdata = data_sitar_D) %>% unname()
data_sitar_V  <- matrix(data_sitar_V, ncol = 3, byrow = T)
data_sitar_V  <- data_sitar_D %>% dplyr::select(-height) %>% dplyr::mutate(height = data_sitar_V[,1])
} else {
  data_sitar_V  <- plot(model_frequ, opt = 'V', xlim = sitar::xaxsd(), ylim = sitar::yaxsd(), returndata = TRUE) 
}
data_sitar_DF <- data_sitar_D %>% data.frame()
...
if(packageVersion('sitar') >= '1.4.0') {
  fh1_ranef_id_max_min <- fh1_ranef_id %>% 
    ...
    dplyr::filter(term == 'a') %>% 
    dplyr::slice(c(which.min(estimate), which.max(estimate))) %>% 
    dplyr::pull(id)
} else {
  fh1_ranef_id_max_min <- fh1_ranef_id %>% 
    dplyr::slice(c(which.min(a), which.max(a))) %>%
...
if(packageVersion('sitar') >= '1.4.0') {
  sitar_ranef_id_min <- fh1_ranef_id %>% 
    dplyr::filter(.data[['level']] %in% ranef_id_max_min[1]) %>% 
    tidyr::pivot_wider(., id_cols = level, names_from = term, values_from = estimate) %>% 
    dplyr::rename(id = level)
  
  sitar_ranef_id_max <- fh1_ranef_id %>% 
    dplyr::filter(.data[['level']] %in% ranef_id_max_min[2]) %>% 
    tidyr::pivot_wider(., id_cols = level, names_from = term, values_from = estimate) %>% 
    dplyr::rename(id = level)
} else {
...
if(packageVersion('sitar') >= '1.4.0') {
  parms_apv_sitar <- round(parms_apv_sitar[2], 2) 
} else {
...
Figure below shows growth curves adjusted via the random effects. As described earlier (see [Introduction]), the random effects align individual-specific growth
trajectories towards a common population average (mean) trajectory. The three growth patterns that *SITAR* adjust via random effects are size, timing and intensity:
...
Adjusted curves aligned with the mean curve indicate
...
round_a <- round(sitar_ranef_id_min$a, 2)
round_b <- round(sitar_ranef_id_min$b, 2)
round_c <- round(sitar_ranef_id_min$c*100, 2)

reround_a <- round(sitar_ranef_id_max$a, 2)
reround_b <- round(sitar_ranef_id_max$b, 2)
reround_c <- round(sitar_ranef_id_max$c*100, 2)
```


## Random effects (size, timing and intensity)
The effect of *SITAR* adjustment in individual curves via random effects is
shown more clearly in Figure below which displays observed and
...
timing and intensity) are provided in the Tables below. Note that intensity c, multiplied by 100, is a
...
effects for the first ten individuals estimated are shown below (see Tables). 
The association (correlation) between random effects (across all individuals) is
shown as a scatterplot in Figures and corresponding estimates are
provided in Tables. Results show that timing is negatively correlated
...
Estimated distance curve (see Figure below) shows that increase in height
follows a sigmoid pattern with maximum gain in height occurring around 12 years
of age. This is confirmed by the velocity curve shown in Figure (below) which
...
Individual specific distance and velocity curves (shown below) follow a pattern similar to the population average
distance and velocity curves described [earlier][Population average (mean)
distance and velocity curves]. Individual velocity curves (shown below) show that all individuals experience a pubertal growth spurt
...
similar to each other (see below), it is not surprising that both
...
intensity (PGV) of the growth spurt (see Tables below).
